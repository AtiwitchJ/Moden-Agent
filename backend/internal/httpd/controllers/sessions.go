...
func (c *SessionsController) delete(w http.ResponseWriter, r *http.Request) {
	...
	envelope.WriteJSON(w, http.StatusOK, DeleteSessionResponse{OK: true, SessionID: id})
}
...
func (c *SessionsController) activity(w http.ResponseWriter, r *http.Request) {
	...
	if err := c.Activity.ApplyActivitySignal(r.Context(), sessionID(r), ports.ActivitySignal{Valid: true, State: state, Detail: strings.TrimSpace(in.Detail)}); err != nil {
...
func (c *SessionsController) spawnOrchestrator(w http.ResponseWriter, r *http.Request) {
	...
	sess, err := c.Svc.SpawnOrchestrator(r.Context(), in.ProjectID, in.Clean, in.Prompt)
	if err != nil {
		envelope.WriteError(w, r, err)
		return
	}
