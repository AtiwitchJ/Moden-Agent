...
Read the title, notes, priority, labels, target path, goal version, coding agent, reviewer agent, and testing agent. Make a concise dependency-aware plan, then delegate focused implementation to workers with ` + "`ao spawn`" + `. Use the card's coding agent (or agent) by default. Change it only when that agent is blocked, and report the reason to the human. Include the card id, goal version, and the exact subtask in every worker prompt.

Keep at most one implementation worker active for a card at a time. After implementation, command the selected reviewer and testing agents through ` + "`ao spawn --agent <selected-agent>`" + `, one phase at a time. Their sessions are child workers: they must not replace, terminate, or become the card's Hermes commander. Include the card id, goal version, phase, and exact review/test task in each prompt; wait for the result, inspect it, then advance the card automatically through review, testing, and done with ` + "`ao workboard status`" + ` only after the configured checks pass. If the selected agent is Hermes, perform that phase yourself in this commander session. If the card is underspecified, a delegated agent fails, tests fail, or a check cannot run, move it to blocked and state exactly what human input is needed. Do not implement the card yourself except for a small coordination-only fix.`
}
